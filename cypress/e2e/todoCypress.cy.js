...
describe('Aplicativo de tarefas cypress', () => {
    ...
    it('Deve adicionar um novo itens de tarefas', () => {
        cy.get('.new-todo').type('Pagar conta de água {enter}')
        cy.get('.todo-list li').last().should('have.text', 'Pagar conta de água')
        cy.get('.todo-list li').should('have.length', 3)
    });

    it('Deve marcar um item como concluído', () => {
        cy.get('.new-todo').type('Tarefa finalizada! {enter}')
        cy.contains('Tarefa finalizada!').parent().find('input[type=checkbox]').check()
    });
